function [A,Rsq,Bounds]=f_Gauss2D(S,Plot,FitOrientation,outputDir,id,row_angle ,col_angle,boundaries2,col_width,row_width)
...
RF_center   = [A(2)*col_width, A(4)*row_width]; %cm
Bounds=[];
for i=1:length(boundaries2)
    xv=boundaries2{i}(:,2)*col_width;
    yv=boundaries2{i}(:,1)*row_width;
    if inpolygon(RF_center(1),RF_center(2),xv,yv)
        Bounds= boundaries2{i};
    end
end
if isempty(Bounds)
    allLengths = cellfun(@length,boundaries2);
    [~,maxLoc] = max(allLengths);
    Bounds = boundaries2{maxLoc};
end
if Plot
    ...
    plot(Bounds(:,2),Bounds(:,1),'r','LineWidth',3);
    %     plot(Boundaries2(:,2),Boundaries2(:,1),'k','LineWidth',3);
    % plot(xRotated, yRotated, 'k', 'LineWidth', 2);
    %     II= imshow(ringimg.*100);
    II=imagesc([0.5 m+0.5],[0.5 n+0.5],ringimg*3);
    II.AlphaData =.3;
    hold off;
    %     set(II, 'AlphaData', rev_rngimg+2)
    %     colormap('hot');
    % Output and compare data and fitted function coefs
    text(+5,n+(n/11),sprintf('Amplitude \t \t  X_0 \t \t \t sigma_X \t \t \t Y_0 \t \t \t \t \t sigma_Y \t \t Angle'),'Color','blue');
    text(+5,n+(n/8),sprintf(' %1.3f \t \t  %1.3f \t \t \t %1.3f \t \t \t %1.3f \t \t \t %1.3f \t \t \t %1.3f',A),'Color','red');
    ...
    plot(xposh,hPoints,'r.',xfit,hfit,'black'); grid on; axis([1,m,dmin,dmax]);
    subplot(4,4,[8,12,16]); xposv = (vy_v-A(4))/cos(A(6))+A(4);
    plot(vPoints,xposv,'g.',vfit,yfit,'black'); grid on; axis([dmin,dmax,1,n]);
