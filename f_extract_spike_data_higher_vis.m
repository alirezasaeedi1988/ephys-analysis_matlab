function f_extract_spike_data_higher_vis(myKsDir,BlDir)
...
% if length(stimulus_point)==2 %#ok<*USENS>
%     session_numbers=[2   1];
%     session_names={'RF';'neon'};
% elseif length(stimulus_point)==3 %#ok<*USENS>
%     session_numbers=[2  3  1];
%     session_names={'RF';'neon';'RF2'};    
% elseif length(stimulus_point)==4 
%     session_numbers=[ 3 1 4];  %% add analysis of opto here too
%     session_names = {'RF';'opto';'rand';'RF2'};
%     session_numbers=circshift(1:4,-1);
%     session_names={'RF';'neon';'Lsize';'Usize'};
% elseif length(stimulus_point)==5
%     session_numbers=[ 2     3     4     5     1];
%     session_names={'RF';'neon';'Lsize';'Usize';'RF2'};
% elseif length(stimulus_point)==6
%     session_numbers=[ 2     3     4     5     1 ];
%     session_names = {'RF';'neon';'Lsize';'Usize';'RF2';'rand'};
% elseif length(stimulus_point)== 7
%     session_numbers=[ 2     3     4     5     1 ];
%     session_names = {'RF';'neon';'Lsize';'Usize';'RF2';'rand'};    
% %     session_names={'RF';'size';'contextual';'size2';'neon';'MAE'};
% elseif length(stimulus_point)==2
%     session_numbers=circshift(1:2,-1);
%     session_names={'RF';'neon'};
% 
% end

if length(stimulus_point)==5%#ok<*USENS>
            session_numbers=[ 5 1 4];  %% add analysis of opto here too
...
for session_number=session_numbers
    ...
    if strcmp(session_name(1:2),'RF')
        Plot =1;
        f_RF_single_unit(myKsDir,BlDir,session_number,session_name,Plot)        
    else 
        ...
        %%%
        Stim_onset_ind=stimulus_point{session_number};                      %starting point of stimulus
        ...
        if strcmp(session_name,'neon')
            ...
            trial_onset_ind=floor(stimulus_point{session_number}-(pst*fs));      %starting point of trials in the data
            nsample=floor(mean(diff(trial_onset_ind)));                          %number of samples for each trials
