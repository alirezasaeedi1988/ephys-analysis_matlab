function f_extract_spike_data_V1full(myKsDir,BlDir)
...
elseif length(stimulus_point)==5
    session_numbers=[5,1];
    session_names={'FRF';'size';'neon';'Fneon';'FRF2'};
...
for session_number=session_numbers
    ...
    if strcmp(session_name(1:3),'FRF')
        Plot =0;
        f_RF_single_unit_V1full(myKsDir,BlDir,session_number,session_name,Plot)
        ...
        %%%
        Stim_onset_ind = floor(stimulus_point{session_number}-(pst*fs));      %starting point of trials in the data
        ...
        %%%
        if strcmp(session_name,'Fneon')
            pst=0.3;                                                 % pre-stimulus time sec
            Stim_onset_ind=floor(stimulus_point{session_number}-(pst*fs));      %starting point of trials in the data
            nsample=floor(mean(diff(Stim_onset_ind)));                          %number of samples for each trials
        ...
        elseif  strcmp(session_name,'rand')
            trial_schedule=trial_schedule(:,1).*(10.^trial_schedule(:,2));
        ...
        for neuron=1:length(good_cid) %%%loop over clusters, spike time for selected cluster is extracted
            ...
            for cond=1:length(condLabels)%%% loop over all condition for selected neuron
                ...
                cond_spt=Stim_onset_ind(ind_trGroup);
                ...
                for trial_number=1:con_Ntrial(cond) %% loop over all trails in a condition for alignment spike times
                    ind_tr= cond_spt(trial_number)<=Neuron_spike_times & Neuron_spike_times<cond_spt(trial_number)+nsample;
