...
for mouse_number=1:length(mouse_numbers)
    ...
    for day=1:length(KS_dirs) %% loop for different days
        ...
        for neuron=1:Nc        %%%loop over clusters in one day
            ...
            if plot_raster
                f_neon6_plot_pref_raster(KS_dirs{day},cond_preferred_angle,psth_locs,spike_data,neuron,...
                    latency_cond_5p(neuron,:),latency_cond2(neuron,:),bin_size,area_names{RF_cell.area(neuron)},session_name,f1f0_cond(neuron,:),f2f0_cond(neuron,:),p_values(neuron,:)) %#ok<*UNRCH>
            end
        ...
        sec_comp     = vertcat(sec_comp,sec_comp_cond);
        f1f0         = vertcat(f1f0,f1f0_cond);
        f2f0         = vertcat(f2f0,f2f0_cond);
...
%% adding opto neurons
opto_data     = load('D:\matwork\Data\neon_opto_laser\New\neon_opto_stat_data.mat');
P_values      = vertcat(P_values,opto_data.P_values(:,1:6));
Cell_area     = vertcat(Cell_area,opto_data.Cell_area); 
ave_act       = vertcat(ave_act,opto_data.ave_act(:,1:6));
ave_act_sweep = vertcat(ave_act_sweep,opto_data.ave_act_sweep(:,1:6));
ave_act_feed  = vertcat(ave_act_feed,opto_data.ave_act_feed(:,1:6));
ave_act2      = vertcat(ave_act2,opto_data.ave_act2(:,1:6));
f1f0          = vertcat(f1f0,opto_data.f1f0(:,1:6));
latency2      = vertcat(latency2,opto_data.latency2(:,1:6));
latency5p     = vertcat(latency5p,opto_data.latency2(:,1:6));
exited        = vertcat(exited,opto_data.exited);
Icell_ind     = vertcat(Icell_ind,opto_data.Icell_ind);
phase_angle   = vertcat(phase_angle,opto_data.phase_angle(:,1:6));
first_comp    = vertcat(first_comp,opto_data.first_comp(:,1:6));
zero_comp     = vertcat(zero_comp,opto_data.zero_comp(:,1:6));
F1_dominant   = vertcat(F1_dominant,opto_data.F1_dominant);
TPlatency     = vertcat(TPlatency,opto_data.TPlatency);
PTratio       = vertcat(PTratio,opto_data.PTratio);
Intrsct_ratio = vertcat(Intrsct_ratio,opto_data.Intrsct_ratio);
Intrsct_ratio_ellips = vertcat(Intrsct_ratio_ellips,opto_data.Intrsct_ratio_ellips);
RF_label      = vertcat(RF_label,opto_data.RF_label);
RF_RSS        = vertcat(RF_RSS,opto_data.RF_RSS);
RF_size_area  = vertcat(RF_size_area,opto_data.RF_size_area);
RF_size       = vertcat(RF_size,opto_data.RF_size);
grand_psth_ncs    = [grand_psth_ncs(:,1:end-10);opto_data.grand_psth_ncs];
grand_psth_ldg    = [grand_psth_ldg(:,1:end-10);opto_data.grand_psth_ldg];
grand_psth_square = [grand_psth_square(:,1:end-10);opto_data.grand_psth_square];

latency2_copy=latency2;
latency5p_copy=latency5p;
%% IGR
...
IGR_nsc_square           = (abs(ave_act(:,1))-abs(ave_act(:,4)))./(abs(ave_act(:,1))+abs(ave_act(:,4))); %ave_act is the mean value of reduced responce (pre-stim reduced)
IGR_nsc_circle           = (abs(ave_act(:,1))-abs(ave_act(:,5)))./(abs(ave_act(:,1))+abs(ave_act(:,5))); %ave_act is the mean value of reduced responce (pre-stim reduced)
ave_IGR1_sweep           = (abs(ave_act_sweep(:,1))-abs(ave_act_sweep(:,2)))./(abs(ave_act_sweep(:,1))+abs(ave_act_sweep(:,2))); %ave_act_sweep is the mean value of reduced responce (pre-stim reduced) for the feedforward sweep (onset to 150 ms)
ave_IGR1_feed            = (abs(ave_act_feed(:,1))-abs(ave_act_feed(:,2)))./(abs(ave_act_feed(:,1))+abs(ave_act_feed(:,2))); %ave_act_feed is the mean value of reduced responce (pre-stim reduced) for the feeadback (150 ms to offset 
feedback_index_NCS       = (abs(ave_act_feed(:,1))-abs(ave_act_sweep(:,1)))./(abs(ave_act_feed(:,1))+abs(ave_act_sweep(:,1))); 
feedback_index_LDG       = (abs(ave_act_feed(:,2))-abs(ave_act_sweep(:,2)))./(abs(ave_act_feed(:,2))+abs(ave_act_sweep(:,2))); 
%% NRERF1
nrelf1          = (first_comp(:,1)-first_comp(:,2))./(first_comp(:,1)+first_comp(:,2));
...
%% grand PSTH
        ...
        grand_psth_ldg1   = grand_psth_ldg;
        grand_psth_ncs1   = grand_psth_ncs;
        grand_psth_squ1   = grand_psth_square;
        
        opto_psth_ldg     = opto_data.opto_psth_ldg;
        opto_psth_ncs     = opto_data.opto_psth_ncs;
        %%smoothing
        grand_psth_ldg1   = smoothdata(grand_psth_ldg1,2,'gaussian',25);%%% In 'smoothdata' the SD is fixed to 1/5th of window width
        grand_psth_ncs1   = smoothdata(grand_psth_ncs1,2,'gaussian',25);%%% In 'smoothdata' the SD is fixed to 1/5th of window width
        grand_psth_squ1   = smoothdata(grand_psth_squ1,2,'gaussian',25);%%% In 'smoothdata' the SD is fixed to 1/5th of window width
        opto_psth_ncs     = smoothdata(opto_psth_ncs,2,'gaussian',25);%%% In 'smoothdata' the SD is fixed to 1/5th of window width
        opto_psth_ldg     = smoothdata(opto_psth_ldg,2,'gaussian',25);%%% In 'smoothdata' the SD is fixed to 1/5th of window width
        % normalization
        max_ldg = max(abs(grand_psth_ldg1(:,onset_ind:offset_ind)),[],2)+1;
        pre_ldg = mean(grand_psth_ldg1(:,1:onset_ind),2);
        pre_ncs = mean(grand_psth_ncs1(:,1:onset_ind),2);
        pre_squ = mean(grand_psth_squ1(:,1:onset_ind),2);
        
        grand_psth_ldg1  = (grand_psth_ldg1-pre_ldg)./max_ldg;
        grand_psth_ncs1  = (grand_psth_ncs1-pre_ncs)./max_ldg;
        grand_psth_squ1  = (grand_psth_squ1-pre_squ)./max_ldg;
        
        opto_start = length(max_ldg)-length(opto_psth_ldg)+1;
        non_pv_idx     = opto_data.ave_act(:,7)< opto_data.ave_act(:,1)*1.5 | opto_data.Cell_area==1;
        non_pv_idx     = [false(opto_start-1,1); non_pv_idx];
        
        opto_psth_ldg    = opto_psth_ldg./max_ldg(opto_start:end);
        opto_psth_ncs    = opto_psth_ncs./max_ldg(opto_start:end);
        
        ave_FR_tonic_NCS = mean(grand_psth_ncs1(:,onset_ind+transient_nbins:offset_ind),2);
        ave_FR_tonic_LDG = mean(grand_psth_ldg1(:,onset_ind+transient_nbins:offset_ind),2);
        ave_FR_tonic_SQU = mean(grand_psth_squ1(:,onset_ind+transient_nbins:offset_ind),2);
        relative_tonic   = (ave_FR_tonic_NCS-ave_FR_tonic_LDG);
        relative_tonic_ncs_sq   = (ave_FR_tonic_NCS-ave_FR_tonic_SQU);
        
        ave_FR_sweep_NCS = mean(grand_psth_ncs1(:,onset_ind:onset_ind+transient_nbins),2);
        ave_FR_sweep_LDG = mean(grand_psth_ldg1(:,onset_ind:onset_ind+transient_nbins),2);
        ave_FR_sweep_SQU = mean(grand_psth_squ1(:,onset_ind:onset_ind+transient_nbins),2);
        relative_sweep   = (ave_FR_sweep_NCS-ave_FR_sweep_LDG);
        relative_sweep_ncs_sq   = (ave_FR_sweep_NCS-ave_FR_sweep_SQU);
...
both_evoked    = evoke_neon_ind & evoke_phy_ind;
only_ldg       = evoke_phy_ind & ~evoke_neon_ind;
only_ncs      = ~evoke_phy_ind & evoke_neon_ind;

excited_idx      = logical(exited);
LDG_excited_idx  = evoke_phy_ind & ave_act(:,2)>=0;
NCS_excited_idx  = evoke_neon_ind & ave_act(:,1)>=0;

both_excited_idx = both_evoked & ave_act(:,2)>=0;
com_ldg_excited  = LDG_excited_idx & f1f0(:,2)<=0;

evoked_any    = (sum(P_values<pv,2)~=0);
...
only_ncs_v1 = sum(only_ncs & Cell_area==1 & NCS_excited_idx)/total_v1_evoked;
only_ncs_lm = sum(only_ncs & Cell_area==2 & NCS_excited_idx)/total_lm_evoked;
only_ncs_li = sum(only_ncs & Cell_area==3 & NCS_excited_idx)/total_li_evoked;
only_ncs_ll = sum(only_ncs & Cell_area==4 & NCS_excited_idx)/total_ll_evoked;
only_NCS_evoked_percent=[only_ncs_v1,only_ncs_lm,only_ncs_li,only_ncs_ll]*100;

only_ldg_v1 = sum(only_ldg & Cell_area==1 & LDG_excited_idx)/total_v1_evoked;
only_ldg_lm = sum(only_ldg & Cell_area==2 & LDG_excited_idx)/total_lm_evoked;
only_ldg_li = sum(only_ldg & Cell_area==3 & LDG_excited_idx)/total_li_evoked;
only_ldg_ll = sum(only_ldg & Cell_area==4 & LDG_excited_idx)/total_ll_evoked;
only_LDG_evoked_percent = [only_ldg_v1,only_ldg_lm,only_ldg_li,only_ldg_ll]*100;

only_evoked_percent = [only_NCS_evoked_percent;only_LDG_evoked_percent];
...
%% edge selectives  
% LDG_edge_selective = f2f0(:,2)>0 & ~isoutlier(f2f0(:,2))& f1f0(:,2)<0 & evoke_phy_ind & excited_idx;
% sum(LDG_edge_selective)
% NCS_edge_selective = f2f0(:,1)>0 & ~isoutlier(f2f0(:,1)) & f1f0(:,1)<0 & evoke_neon_ind & excited_idx;
% sum(NCS_edge_selective)
% both_edge_selective = NCS_edge_selective & LDG_edge_selective;
% sum(both_edge_selective)

save([data_folder,'/neon_stat_data.mat'])
